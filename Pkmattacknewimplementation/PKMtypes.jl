...
angreifer = A[2:end, 1]        # Zeilenbeschriftungen
verteidiger = A[1, 2:end]      # Spaltenbeschriftungen

for i in 1:size(daten,1)
    for j in 1:size(daten,2)
        wert = daten[i,j]
        beschreibung = ""
        if wert == 2
        ...
        println("Angreifer = $(angreifer[i]), Verteidiger = $(verteidiger[j]) -> $beschreibung")
    end
end
